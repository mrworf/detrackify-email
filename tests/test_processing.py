...
def test_guard_whitelist_sender():
    """
    Test that sender whitelisting bypasses guard mode for trusted senders.
    
    Expected outcome: When sender matches whitelist regex, no links should be
    rewritten even in 'always' mode, allowing trusted senders to bypass protection.
    """
    msg = process_email(GUARD_FILE, [
        "--guardserver", SERVER,
        "--guardsalt", SALT,
...
def test_guard_whitelist_link():
    """
    Test that link whitelisting bypasses guard mode for trusted URLs.
    
    Expected outcome: Links matching whitelist regex should not be rewritten,
    allowing trusted domains to bypass guard protection.
    """
    msg = process_email(GUARD_FILE, [
        "--guardserver", SERVER,
        "--guardsalt", SALT,
